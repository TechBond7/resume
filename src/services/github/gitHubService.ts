...
export const gitHubService: IGitHubService = {
    getRepos: () => fetch(reposUrl).then(response => response.json()).then(data => repositoriesStore.repos = data).catch(error => console.log(error)),
    getUpdatedRepo: () => {
        if (repositoriesStore.repos.length)
            repositoriesStore.repo = repositoriesStore.repos.reduce((current, next) => new Date(current.pushed_at) > new Date(next.pushed_at) ? current : next, repositoriesStore.repos[0]).name;
    },
    getLastCommit: () => {
        if (repositoriesStore.repos.length)
            fetch(commitsUrl.replace("replaceMe", repositoriesStore.repo)).then(response => response.json()).then(data => {
